import { Interface, PageResponse, InterfaceMapping } from '../types';
...
class InterfaceService {
  ...
  async getAllInterfaces(
    ...
    try {
      ...
      const response = await api.get<PageResponse<Interface>>(ENDPOINTS.INTERFACES.BASE, { params });
      return response.data;
    } catch (error) {
      this.handleServiceError(error, 'fetching interfaces');
    }
  }

  async getInterfacesByClientId(clientId: number): Promise<Interface[]> {
    try {
      console.log('interfaceService: Fetching interfaces for client:', clientId);
      setClientContext(clientId);
      console.log('interfaceService: Client context set, making API request...');
      const response = await api.get<Interface[]>(ENDPOINTS.INTERFACES.CLIENT_INTERFACES(clientId));
      console.log('interfaceService: API response:', response);
      if (!response.data) {
        console.warn('interfaceService: No data in response');
        return [];
      }
      console.log('interfaceService: Returning interfaces:', response.data);
      return response.data;
    } catch (error: any) {
      console.error('interfaceService: Error details:', {
        error,
        message: error?.message,
        response: error?.response?.data,
        status: error?.response?.status
      });
      this.handleServiceError(error, 'fetching interfaces by client');
      return [];
    }
  }
  ...
  async getInterfaceById(id: number): Promise<Interface> {
    try {
      ...
      this.handleServiceError(error, 'fetching interface by ID');
    }
  }
  ...
  async createInterface(interfaceData: Omit<Interface, 'id'>): Promise<Interface> {
    try {
      ...
      if (error instanceof Error && !error.message.includes('fetching')) {
        // If it's a validation error, throw it directly
        throw error;
      }
      this.handleServiceError(error, 'creating interface');
    }
  }
  ...
  async updateInterface(id: number, interfaceData: Partial<Interface>): Promise<Interface> {
    try {
      ...
      this.handleServiceError(error, 'updating interface');
    }
  }
  ...
  async deleteInterface(id: number): Promise<void> {
    try {
      ...
      this.handleServiceError(error, 'deleting interface');
    }
  }
  ...
  async getInterfacesByStatus(
    ...
    try {
      ...
      this.handleServiceError(error, 'fetching interfaces by status');
    }
  }
  ...
  async getInterfaceMappings(interfaceId: number): Promise<InterfaceMapping[]> {
    try {
      ...
      this.handleServiceError(error, 'fetching interface mappings');
    }
  }

  async updateInterfaceMappings(id: number, mappings: InterfaceMapping[]): Promise<InterfaceMapping[]> {
    try {
      const response = await api.put<InterfaceMapping[]>(ENDPOINTS.INTERFACES.MAPPINGS(id), mappings);
      return response.data;
    } catch (error) {
      this.handleServiceError(error, 'updating interface mappings');
    }
  }
