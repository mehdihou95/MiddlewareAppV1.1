...
public class AuditLogAspect {
    ...
    private void createAuditLog(ProceedingJoinPoint joinPoint, Object result, Exception exception, long startTime) {
        ...
        com.xml.processor.annotation.AuditLog auditLogAnnotation = signature.getMethod().getAnnotation(com.xml.processor.annotation.AuditLog.class);

        AuditLog auditLog = new AuditLog();
        auditLog.setAction(auditLogAnnotation.value());
        ...
        // Set entity type based on the result
        if (result instanceof com.xml.processor.model.User) {
            auditLog.setEntityType("USER");
            auditLog.setEntityId(((com.xml.processor.model.User) result).getId());
        } else if (joinPoint.getArgs().length > 0 && joinPoint.getArgs()[0] instanceof Long) {
            // For delete operations where we have the ID as first argument
            auditLog.setEntityType("USER");
            auditLog.setEntityId((Long) joinPoint.getArgs()[0]);
        } else {
            // Default values for when entity information is not available
