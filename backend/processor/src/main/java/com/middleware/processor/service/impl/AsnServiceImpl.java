...
import com.middleware.processor.config.ClientContextHolder;
import com.middleware.processor.model.AsnHeader;
import com.middleware.processor.model.AsnLine;
import com.middleware.processor.repository.AsnHeaderRepository;
import com.middleware.processor.repository.AsnLineRepository;
...
public class AsnServiceImpl implements AsnService {
    ...
    public Optional<AsnHeader> findByAsnNumberAndClient_Id(String asnNumber, Long clientId) {
        return asnHeaderRepository.findByAsnNumberAndClient_Id(asnNumber, clientId);
    }
    ...
    public Page<AsnHeader> findByClient_IdAndReceiptDttmBetween(Long clientId, LocalDate startDate, LocalDate endDate, Pageable pageable) {
        return asnHeaderRepository.findByClient_IdAndReceiptDttmBetween(clientId, startDate, endDate, pageable);
    }
    ...
    public AsnHeader updateAsnHeader(Long id, AsnHeader headerDetails) {
        ...
        header.setAsnNumber(headerDetails.getAsnNumber());
        header.setAsnType(headerDetails.getAsnType());
        header.setBusinessPartnerId(headerDetails.getBusinessPartnerId());
        header.setBusinessPartnerName(headerDetails.getBusinessPartnerName());
        header.setReceiptDttm(headerDetails.getReceiptDttm());
        header.setAsnLevel(headerDetails.getAsnLevel());
    ...
    public List<AsnLine> createAsnLines(List<AsnLine> lines) {
        List<AsnLine> savedLines = new ArrayList<>();
        for (AsnLine line : lines) {
            savedLines.add(asnLineRepository.save(line));
        }
        return savedLines;
    }
} 
